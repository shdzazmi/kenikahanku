    ...
    this.init = function () {
      ...
      if (!self.ispage || !cap.isieold) {
        ...
        if (cap.cantouch || opt.emulatetouch) {
          ...
          var _checkSelectionScroll = function checkSelectionScroll(e) {
    ...
    if (!opt.smoothscroll) {
      ...
      this.cursorupdate = {
        ...
        start: function start() {
          ...
          var _loop = function loop() {
            if (m.running) setAnimationFrame(_loop);
            self.showCursor(self.getScrollTop(), self.getScrollLeft());
            self.notifyScrollEvent(self.win[0]);
          };
          setAnimationFrame(_loop);
        },
        stop: function stop() {
          this.running = false;
        }
      ...
      this.doScrollPos = function (x, y, spd) {
        ...
        var _loop2 = function loop() {
          ...
          if (x <= 1) {
            self.timer = setAnimationFrame(_loop2);
          } else {
            self.scrollrunning = false;
            self.timer = 0;
            self.triggerScrollEnd();
    ...
    this.doMomentum = function (gp) {
      ...
      if (sy || sx) {
        ...
        var _onscroll = function onscroll() {
          ...
          if (self.demulxy < 1) {
            self.timer = setTimeout(_onscroll, tm);
          } else {
            self.stop();
            self.nc.hideCursor();
            self.doSnapy(nx, ny);
